...
function calcul(from, to, t, params, results)
    ...
    for _, value in ipairs(t) do
        ...
        if flag and t.checked then
          flag = (t.checked == params.checked)
        end
        if flag and t.level then
          flag = (t.level == params.level)
        end
        if flag and t.cell then
          flag = (t.cell == params.cell)
        end
        if flag and t.category then
          flag = (t.category == params.category)
        end
        if flag and t.cardtype then
          flag = (t.cardtype == params.cardtype)
        end
        if flag and t.cardlike then
          flag = not (params.card:gmatch(t.cardlike) == nil)
        end
        if flag and t.hintlike then
          flag = not (params.hint:gmatch(t.hintlike) == nil)
        end
